    ...
    public function up(): void
    {
        Schema::create('dash_videos',
            function (Blueprint $table) {
                $table->id();
                $table->foreignUuid('virtual_file_uuid')->constrained('virtual_files', 'uuid')->onDelete('cascade');//->comment('關聯檔案');
                $table->foreignId('share_table_virtual_file_id')
				  ->references('id')->on('share_table_virtual_file')
				  // 改用 NO ACTION（或 RESTRICT），避免多重 cascade
				  ->onDelete('no action')
				  // 如果需要也可以保留 update cascade
				  ->onUpdate('cascade');
                //$table->foreignId('member_id')->constrained()->onDelete('cascade');//->comment('關聯擁有者');
        ...
        Schema::table('share_table_virtual_file', function (Blueprint $table) {
            $table->foreignId('dash_videos_id')->change()->constrained('dash_videos')->onDelete('no action');//->comment('關聯 dash_videos 表');
        });
    }
