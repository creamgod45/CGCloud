...
class I18N
{
    ...
    /**
     * @param ELanguageCode|null $languageCode 設定現在語言狀態並且讀取語言檔案並覆蓋 Set the current language status and read the
     *                                         language file and overwrite it
    ...
    public function __construct(
        ?ELanguageCode $languageCode = ELanguageCode::zh_TW,
    ...
    {
        if (file_exists("I18N.lock") && !$CompileMode) {
            return;
        }
    ...
    {
        if (empty($this->languageTextList)) {
            return null;
        }
        if (empty($this->languageTextList[$elanguageText->name])) {
            return null;
        }
        if ($toCGString) {
