...
class Utils
{
    ...
    public static function getRandomHexColor($hash = true)
    {
        if ($hash) {
    ...
    public static function getContrastColor($hexColor)
    {
        ...
        $brightness = (($r * 299) + ($g * 587) + ($b * 114)) / 100000;
    ...
    public static function get_browser($user_agent)
    {
        ...
        elseif (strpos($t, 'crawler') || strpos($t, 'api') || strpos($t, 'spider') || strpos($t, 'http') || strpos($t,
                'bot') || strpos($t, 'archive') || strpos($t, 'info') || strpos($t, 'data')) {
            return '[Bot] Other';
        }
     ...
     */
    public static function exploreDirectory(
        $path,
        bool $CGString = false,
        bool $deep = false,
        bool $fileOnly = false,
        bool $hideFileSearchable = true,
        array $filterFileExtension = [],
        int $filterSizeLimit = -1,
        array $filterMIMEType = [],
        bool $filterPathName = false,
    ): array {
        $originFilePath = "";
        $results = [];
