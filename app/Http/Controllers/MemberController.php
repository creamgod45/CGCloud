...
class MemberController extends Controller
{
    ...
    public function forgetPasswordPost(Request $request)
    {
        ...
        if ($v instanceof MessageBag) {
            ...
            if (Cache::has($cacheKey)) {
                ...
                ResetPassword::toMailUsing(function (Member $member, $token) use ($cgLCI) {
                    // Log::info("run1");
                    $i18N = $cgLCI->getI18N();
                    $m = (new MailMessage)
                        ->success()
        ...
        if ($v instanceof MessageBag) {
            ...
            switch ($v['method']) {
                ...
                case 'password':
                    ...
                    break;
                case 'avatar':
                    if (! isset($v['avatar_uuid'])) {
                        return response()->json(['message' => '缺少頭像 UUID'], ResponseHTTP::HTTP_BAD_REQUEST);
                    }

                    return $this->profilepost_avatar($v, $i18N);
            }
        }
    ...
    private function profilepost_avatar(array $v, I18N $i18N): JsonResponse
    {
        $member = Auth::user();
        $uuid = $v['avatar_uuid'];

        $vf = VirtualFile::where('uuid', $uuid)->first();
        if (! $vf) {
            return response()->json(['message' => '檔案不存在'], ResponseHTTP::HTTP_NOT_FOUND);
        }

        // 移除舊頭像
        $oldAvatars = VirtualFile::where('members_id', $member->id)
        ...
        // 移動新檔案並更新狀態
        $newDir = 'Profile/Avatars/'.$member->id;
        $newFilename = $uuid.'.'.$vf->extension;
        $newPath = $newDir.'/'.$newFilename;

        Storage::disk($vf->disk)->makeDirectory($newDir);
        Storage::disk($vf->disk)->move($vf->path, $newPath);
        ...
        return response()->json([
            'message' => '大頭照已成功更新',
            'avatar_url' => $vf->getTemporaryUrl(now()->addDay()),
        ]);
    }
